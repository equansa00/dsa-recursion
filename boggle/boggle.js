...
function find(board, word) {
  ...
  function isWord(board, word, row, col, visited) {
    ...
    for (let dir of directions) {
      if (isWord(board, word.slice(1), row + dir[0], col + dir[1], visited)) {
        return true;
      }
    }
  ...
  for (let row = 0; row < board.length; row++) {
    for (let col = 0; col < board[0].length; col++) {
      if (
        board[row][col] === word[0] &&
        isWord(board, word, row, col, Array.from({ length: board.length }, () => Array(board[0].length).fill(false)))
      ) {
        return true;
      }
    }
